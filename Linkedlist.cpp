...
bool addAfter(List*& L, int data, int val)
{
    if (L->p_tail == nullptr) return false;
    NODE* cur = L->p_head;
    while (cur != nullptr && cur->key != val) {
        cur = cur->p_next;
    }
...
void removeNODE(List*& L, NODE* temp) {
    if (L->p_head == nullptr || temp == nullptr) return;
...
int main()
{
    ...
    std::cout << "--- SINGLY-LINKEDLIST ---" << std::endl;
