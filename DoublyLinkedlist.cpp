...
void removeBefore(List*& L, int val)
{
    if (L->p_head == nullptr || L->p_head == L->p_tail) return;
    NODE* cur=L->p_head;
    NODE* dele=nullptr;
    NODE* predele=nullptr;
...
{
    NODE* cur = L->p_head;
    if (cur == nullptr||cur->p_next==L->p_tail) return;
    NODE* nex = cur->p_next;
    while (nex != nullptr&&cur->key!=val) {
        cur = cur->p_next;
        nex = nex = nex->p_next;
    }
    if (nex == nullptr) return;
    else if (nex == L->p_tail) {
        L->p_tail = cur;
        cur->p_next = nullptr;
        delete nex;
        return;
        ...
        cur->p_next = nex->p_next;
        nex->p_next->p_prev = cur;
        delete nex;
        return;
    }
}
...
void removePos(List*& L, int data, int pos)
{
    if (L->p_head == nullptr || pos < 0) return;
...
List* reverseList(List* L)
{
 ...
 void removeNODE(List* L, NODE* temp) {
    if (temp == L->p_head) removeHead(L);
    else if (temp == L->p_tail) removeTail(L);
    else {
        NODE* pre = temp->p_prev;
        NODE* nex = temp->p_next;
